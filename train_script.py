...
def do_compute_metrics(probas_pred, target):
    ...
    auc_prc = metrics.average_precision_score(target, probas_pred)
...
def train(model, train_data_loader, val_data_loader, loss_fn,  optimizer, n_epochs, device, scheduler=None):
    ...
    for i in range(1, n_epochs+1):
        ...
        with torch.no_grad():
            ...
            train_acc, train_auc_roc, train_auc_prc, train_f1_score = do_compute_metrics(train_probas_pred, train_ground_truth)
            ...
            val_acc, val_auc_roc, val_auc_prc, val_f1_score = do_compute_metrics(val_probas_pred, val_ground_truth)
        ...
        print(f'\t\ttrain_roc: {train_auc_roc:.4f}, val_roc: {val_auc_roc:.4f}, train_auprc: {train_auc_prc:.4f}, val_auprc: {val_auc_prc:.4f}, train_f1: {train_f1_score:.4f}, val_f1:　{val_f1_score:.4f}')
        # print(f'{val_acc:.4f} {val_auc_roc:.4f} {val_auc_prc:.4f} {val_f1_score:.4f}')
    # model_path = 'data/ddi-model.pt'
    # torch.save(model.state_dict(), model_path)
    # print("model has been saved to %s." % (model_path))
model = models.MASMDDI(n_atom_feats, n_atom_hid, kge_dim, rel_total, num_layers, weight_conv='WeightConv1', multi_channel='False')
